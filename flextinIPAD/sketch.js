...
function draw() {
  ...
  image(video, 0, 0);
  image(logo, 712, 530);
  image(robotito, 825, 325,130,128);
  fill("white");
  textSize(17);
  text("IES MARÍA BLASCO", 835,28);
  textSize(15);
  text("San Vicente del Raspeig",835,50)
  textSize(25);
  text("Dpto. Tecnología",800,485);
  text("2026", 870, 510);
  image(blasco, 760, 5,58,50);
  image(mujer, -120, 17);
  if (poses.length > 0) {
    let pose = poses[0];
    ...
    if (alineado(poses[0].left_ankle.x, poses[0].left_ankle.y, poses[0].left_knee.x, poses[0].left_knee.y, poses[0].left_hip.x, poses[0].left_hip.y)) {
      ...
      fill("white");
      textSize(16);
      text("rodilla Y: " + round(pose.left_knee.y), 791, 215);
      text("rodilla X: " + round(pose.left_knee.x), 791, 235);
      text("tobillo Y: " + round(pose.left_ankle.y), 791, 255);
      text("tobillo X: " + round(pose.left_ankle.x), 791, 275);
      text("cadera Y: " + round(pose.left_hip.y), 791, 295);
      text("cadera X: " + round(pose.left_hip.x), 791, 315);
    }
  }

  fill("rgb(247,247,247)");
  textSize(18);
  text("Alumno/a:", 790, 90);
  text("Grupo:", 790, 150);
  fill('white');
  textSize (15);
